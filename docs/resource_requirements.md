...
- `[CODE][OBSERVED]` DATA-11 已在两个远程 canonical 输入上完成：848 个 MECAT samples（807 个唯一来源视频）与固定 WavCaps AudioSet_SL metadata 间得到 4 个同源 YouTube video candidates；call/audit/wrapper 均为 0，统计和候选工件 SHA256 已固定。该结果仅为来源 provenance，不证明时间片/音频重复，不能解除 EXP-09。
...
| Vanilla backbones | 三个 base revision、base-only 锁流水线和 Clotho 正式生成器已固定；Nemotron 正式评测完成；Qwen3B 锁复用已审计 base inventory | Qwen3B GPU smoke/full 待批准；Qwen7B base-only 锁仍缺；AudioCaps/MECAT 候选集仍缺 |
