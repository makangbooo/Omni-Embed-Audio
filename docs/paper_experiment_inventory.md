...
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（发布 UIQ 为 848，需澄清） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | README 把 Hydra `key=value` 语法传给 argparse CLI，命令会报参数错误；复现分支已用 `run_qwen3b_clotho_retrieval_suite.sh` 固定首个 Qwen3B-Cl/Clotho 的 default-joint all-caption 与 T2A-only seed-0 两条 `[CODE]` 路径；完整 13×3 矩阵仍未运行 | 13 模型/权重、3 数据集、统一候选集 | C（需评测入口补全） | TODO |
