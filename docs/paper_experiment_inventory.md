...
| EXP-12 | 附录 G.1；表 12 | Question UIQ T2A | AudioCaps/Clotho/MECAT-UIQ | 4 CLAP + 6 OEA | 每数据集 R@1/5/10 | 上游 README/CLI 不兼容；复现分支已提供锁绑定 GPU 生成和 CPU 四协议套件 | Qwen3B-Cl/Clotho=`25.8373/54.0670/66.6029`（论文 `25.74/54.26/66.79`）；Qwen3B-AC/Clotho=`21.5311/44.4976/59.5215`（论文 `21.34/44.11/59.71`）；其余模型/数据待准备 | B（数据公开，需兼容补丁） | IN_PROGRESS |
