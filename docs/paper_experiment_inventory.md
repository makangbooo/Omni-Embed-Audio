...
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（发布 UIQ 为 848，需澄清） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | `bash scripts/run_qwen3b_cl_clotho_embeddings.sh` + `bash scripts/run_qwen3b_clotho_retrieval_suite.sh <embedding_dir>` 已真实完成首个 Qwen3B-Cl/Clotho 闭环；`[CODE]` all-caption 得 `22.7368/49.4737/63.3110`，seed0 one-caption 得 `21.9139/49.7608/64.2105`；完整 13×3 矩阵仍未运行 | 其余 12 模型/权重、AudioCaps/MECAT 候选集、论文 caption 选择 `[MISSING]` | C（首个分栏闭环完成） | IN_PROGRESS |
| EXP-11 | §5.2；表 3 | caption Text-to-Text 完整基线 | 同 EXP-10 captions/candidates | 同 EXP-10 | R@1/5/10 | 同一真实 suite 的 seed0 one-caption `[CODE]` 得 `64.4019/76.1722/80.7656`，all-caption `[INFERRED]` 敏感性得 `64.3636/75.1388/79.7321`；四协议均预声明并分栏，未按接近度择优 | 其余模型/数据；论文 caption 选择、self-exclusion、tie 口径 `[MISSING]` | C（首个分栏闭环完成） | IN_PROGRESS |
| EXP-12 | 附录 G.1；表 12 | Question UIQ T2A | AudioCaps/Clotho/MECAT-UIQ | 4 CLAP + 6 OEA | 每数据集 R@1/5/10 | 上游 README/CLI 不兼容；复现分支已提供 `run_qwen3b_cl_clotho_positive_uiq_embeddings.sh` 与 `run_qwen3b_clotho_positive_uiq_suite.sh`，固定首个 Qwen3B-Cl/Clotho Question 路径 | Qwen3B-Cl/Clotho 正式结果 `25.8373/54.0670/66.6029`，相对论文 `25.74/54.26/66.79`；其余模型/数据待准备 | B（数据公开，需兼容补丁） | IN_PROGRESS |
| EXP-13 | 附录 G.2；表 13 | Imperative UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一固定生成/评测套件显式物化 Imperative 的 1,045 个 query indices | Qwen3B-Cl/Clotho 正式结果 `25.8373/55.6938/66.9856`，相对论文 `25.45/55.69/67.56`；其余同 EXP-12 | B | IN_PROGRESS |
| EXP-14 | 附录 G.3；表 14 | Paraphrase UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一固定生成/评测套件显式物化 Paraphrase 的 1,045 个 query indices | Qwen3B-Cl/Clotho 正式结果 `26.9856/55.1196/70.2392`，相对论文 `27.56/55.50/70.81`；其余同 EXP-12 | B | IN_PROGRESS |
| EXP-15 | 附录 G.4；表 15 | Keyphrase/`tagging` UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一套件保留 `[CODE] tagging` 与 `[PAPER] Keyphrase` 双标签并显式物化 1,045 个 query indices | Qwen3B-Cl/Clotho 正式结果 `27.5598/58.0861/71.6746`，相对论文 `27.66/57.99/71.87`；其余同 EXP-12 | B | IN_PROGRESS |
