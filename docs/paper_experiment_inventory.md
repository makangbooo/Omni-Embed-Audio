...
| EXP-18 | §5.4；附录 H；表 5、16 | 推理效率 | Clotho 1,045 clips | LAION/MGA/M2D + 3 OEA | audio ms/clip、text ms/query、peak GPU GB | `[OBSERVED]` Qwen3B(+Cl) 已完成 A100 与 RTX 4090；Nemo3B(+Cl) 已完成 RTX 4090 受控 benchmark；`[MISSING]` 论文 batch/warmup/timing/memory 口径，不能提升为严格表 5/16 复现 | 已固定 Qwen3B(+Cl)/Nemo3B(+Cl) 受控结果；仍需 LAION/MGA/M2D/Qwen7B 四个模型，固定论文协议继续 `[MISSING]` | C（controlled-only） | IN_PROGRESS |
| EXP-19 | §3.1/§5.4；表 5、16 | 训练参数量/占比 | 不依赖数据 | 3 OEA + 3 CLAP | trainable params、占总参数百分比 | `[CHECKPOINT][OBSERVED]` Qwen3B(+Cl) 与 Nemo3B(+Cl) 均为 LoRA=12.61568M、双 head 各 1.04960M、合计 14.71488M；`[MISSING]` 论文统计口径 | 已固定两个 OEA +Cl checkpoint 实测分栏；仍需其余模型结构/权重，不能推断论文 16.2/13.7M 口径 | B（controlled-only） | IN_PROGRESS |
