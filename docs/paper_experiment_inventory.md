...
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（发布 UIQ 为 848，需澄清） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | Qwen3B-Cl、Qwen3B-AC 与 Nemo3B-Cl 在 Clotho 上均完成全量 embedding 和双协议 CPU 闭环；Nemo3B-Cl `[CODE]` all-caption=`21.7225/47.1196/60.4402`、seed0=`21.6268/46.7943/59.8086`，论文=`21.57/47.16/60.36` | 其余 10 个模型/权重、AudioCaps/MECAT 候选集、论文 caption 选择 `[MISSING]` | C（三个 OEA 分栏闭环完成） | IN_PROGRESS |
| EXP-11 | §5.2；表 3 | caption Text-to-Text 完整基线 | 同 EXP-10 captions/candidates | 同 EXP-10 | R@1/5/10 | Qwen3B-AC、Qwen3B-Cl 与 Nemo3B-Cl 的 `[CODE]` seed0 和 `[INFERRED]` all-caption 均已分栏；Nemo3B-Cl seed0=`62.9665/75.9809/80.8612`、all-caption=`63.6938/75.2344/80.0191`，论文=`63.77/75.29/80.11`；全部协议均预声明，未按接近度择优 | 其余模型/数据；论文 caption 选择、self-exclusion、tie 口径 `[MISSING]` | C（三个 OEA 分栏闭环完成） | IN_PROGRESS |
| EXP-12 | 附录 G.1；表 12 | Question UIQ T2A | AudioCaps/Clotho/MECAT-UIQ | 4 CLAP + 6 OEA | 每数据集 R@1/5/10 | 上游 README/CLI 不兼容；复现分支以锁绑定 GPU 生成和 CPU 四协议套件完成三个 OEA/Clotho 单元；Nemo3B-Cl generation/suite 均 complete，4,180/4,180 queries、六个发现的 exit code 均为 0 | Qwen3B-Cl=`25.8373/54.0670/66.6029`（论文 `25.74/54.26/66.79`）；Qwen3B-AC=`21.5311/44.4976/59.5215`（论文 `21.34/44.11/59.71`）；Nemo3B-Cl=`23.6364/50.8134/63.9234`（论文 `23.44/50.62/63.73`）；均为 Clotho `[CODE] close`，其余模型/数据待准备 | B（数据公开，需兼容补丁） | IN_PROGRESS |
| EXP-13 | 附录 G.2；表 13 | Imperative UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一固定生成/评测套件显式物化 Imperative 的 1,045 个 query indices；Nemo3B-Cl suite/protocol exit 均为 0 | Qwen3B-Cl=`25.8373/55.6938/66.9856`（论文 `25.45/55.69/67.56`）；Qwen3B-AC=`22.4880/46.2201/59.1388`（论文 `22.30/46.12/59.33`）；Nemo3B-Cl=`24.0191/51.1005/64.9761`（论文 `24.02/51.29/64.69`）；均为 Clotho `[CODE] close`，其余同 EXP-12 | B | IN_PROGRESS |
| EXP-14 | 附录 G.3；表 14 | Paraphrase UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一固定生成/评测套件显式物化 Paraphrase 的 1,045 个 query indices；Nemo3B-Cl suite/protocol exit 均为 0 | Qwen3B-Cl=`26.9856/55.1196/70.2392`（论文 `27.56/55.50/70.81`）；Qwen3B-AC=`20.9569/46.7943/60.0957`（论文 `21.24/46.79/60.19`）；Nemo3B-Cl=`23.5407/49.9522/64.4019`（论文 `23.83/50.14/64.50`）；均为 Clotho `[CODE] close`，其余同 EXP-12 | B | IN_PROGRESS |
| EXP-15 | 附录 G.4；表 15 | Keyphrase/`tagging` UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一套件保留 `[CODE] tagging` 与 `[PAPER] Keyphrase` 双标签并显式物化 1,045 个 query indices；Nemo3B-Cl suite/protocol exit 均为 0 | Qwen3B-Cl=`27.5598/58.0861/71.6746`（论文 `27.66/57.99/71.87`）；Qwen3B-AC=`24.3062/49.6651/61.7225`（论文 `24.50/49.76/61.72`）；Nemo3B-Cl=`25.8373/52.6316/66.1244`（论文 `25.93/52.54/66.03`）；均为 Clotho `[CODE] close`，其余同 EXP-12 | B | IN_PROGRESS |
