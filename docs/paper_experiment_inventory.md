...
| EXP-12 | 附录 G.1；表 12 | Question UIQ T2A | AudioCaps/Clotho/MECAT-UIQ | 4 CLAP + 6 OEA | 每数据集 R@1/5/10 | 上游 README/CLI 不兼容；复现分支已提供 `run_qwen3b_cl_clotho_positive_uiq_embeddings.sh` 与 `run_qwen3b_clotho_positive_uiq_suite.sh`，固定首个 Qwen3B-Cl/Clotho Question 路径 | 正查询已发布；Clotho 音频/模型已具备，正式 embedding 待运行；其余模型/数据待准备 | B（数据公开，需兼容补丁） | IN_PROGRESS |
