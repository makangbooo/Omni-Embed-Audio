...
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（发布 UIQ 为 848，需澄清） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | README 把 Hydra `key=value` 语法传给 argparse CLI，命令会报参数错误；复现分支已固定首个 Qwen3B-Cl/Clotho 两条 T2A 协议，并新增三种 vanilla 的锁绑定、base-hidden-dimension、公开代码 no-audio-prefix Clotho 生成路径；真实 vanilla 锁/GPU embedding 与完整 13×3 矩阵仍未运行 | 13 模型/权重、3 数据集、统一候选集 | C（需评测入口补全） | TODO |
| EXP-11 | §5.2；表 3 | caption Text-to-Text 完整基线 | 同 EXP-10 captions/candidates | 同 EXP-10 | R@1/5/10 | `[CODE] eval_core.py` 有 T2T；复现分支已固定 Qwen3B-Cl/Clotho 的 seed-0 one-caption `[CODE]` 路径和 all-caption `[INFERRED]` 敏感性路径；论文未公开选择口径，完整矩阵未运行 | 13 模型/权重、自动/人工 captions 的确切版本 | C | TODO |
