...
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（发布 UIQ 为 848，需澄清） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | 六个 OEA 变体、vanilla Nemotron-3B 与 vanilla Qwen2.5-Omni-3B 在 Clotho 上均完成全量 embedding 和双协议 CPU 闭环；Qwen3B all-caption=`0.1722/0.6316/1.1483`、论文=`0.17/0.63/1.13`；base-only loader/官方 metric source 且未加载 OEA checkpoint/LoRA/head | AudioCaps 实际音频、MECAT 847/caption 口径、其余 5 个 vanilla/CLAP baseline 和论文 caption 选择仍缺失 | C（八个模型的 Clotho 分栏闭环完成） | IN_PROGRESS |
| EXP-11 | §5.2；表 3 | caption Text-to-Text 完整基线 | 同 EXP-10 captions/candidates | 同 EXP-10 | R@1/5/10 | 六个 OEA/Clotho 变体、vanilla Nemotron-3B 与 vanilla Qwen2.5-Omni-3B 的 `[CODE]` seed0 与 `[INFERRED]` all-caption 均已分栏；Qwen3B seed0=`39.5215/52.2488/59.0431`、all-caption=`38.4115/51.1388/57.1100`，论文=`38.35/51.14/57.07`；未按接近度择优 | 其余模型/数据；论文 caption 选择、self-exclusion、tie 口径 `[MISSING]` | C（八个模型的 Clotho 分栏闭环完成） | IN_PROGRESS |
| EXP-12 | 附录 G.1；表 12 | Question UIQ T2A | AudioCaps/Clotho/MECAT-UIQ | 4 CLAP + 6 OEA | 每数据集 R@1/5/10 | 上游 README/CLI 不兼容；复现分支以官方 OEA loading/metric source 加最小兼容层完成四个 OEA/Clotho 单元；Nemo3B-AC Question=`18.9474/43.6364/56.3636`（论文 `19.81/43.54/57.61`） | Qwen3B-Cl、Qwen3B-AC、Nemo3B-Cl、Nemo3B-AC 均为 Clotho `[CODE] close`；其余模型/数据待准备 | B（数据公开，需兼容补丁） | IN_PROGRESS |
| EXP-13 | 附录 G.2；表 13 | Imperative UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一固定生成/评测套件显式物化 Imperative 的 1,045 个 query indices；Nemo3B-AC=`19.0431/43.3493/56.8421`（论文 `18.85/43.25/57.03`） | 四个已完成 OEA/Clotho 单元均为 `[CODE] close`，其余同 EXP-12 | B | IN_PROGRESS |
| EXP-14 | 附录 G.3；表 14 | Paraphrase UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一固定生成/评测套件显式物化 Paraphrase 的 1,045 个 query indices；Nemo3B-AC=`20.7656/44.3062/56.6507`（论文 `20.96/44.50/57.22`） | 四个已完成 OEA/Clotho 单元均为 `[CODE] close`，其余同 EXP-12 | B | IN_PROGRESS |
| EXP-15 | 附录 G.4；表 15 | Keyphrase/`tagging` UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | 同一套件保留 `[CODE] tagging` 与 `[PAPER] Keyphrase` 双标签；Nemo3B-AC=`21.5311/45.8373/60.1914`（论文 `21.53/45.45/59.14`） | 四个已完成 OEA/Clotho 单元均为 `[CODE] close`，其余同 EXP-12 | B | IN_PROGRESS |
