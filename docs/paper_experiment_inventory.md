...
| EXP-06 | 附录 B.1；表 6 | 数据来源与潜在污染关系审计 | WavCaps 子集与 7 个评测集 | 文件/来源匹配 | 来源对应关系 | `[CODE][OBSERVED]` DATA-05 已固定 MECAT 848-row manifest；DATA-09 已实现 AudioCaps/Clotho metadata provenance | 固定 WavCaps/AudioCaps/Clotho metadata；MECAT manifest；远程 DATA-08/09 正式复算 | B | IN_PROGRESS |
...
| EXP-09 | §4.1；附录 B.4 | MECAT–WavCaps 无显著重叠核验 | MECAT 与 WavCaps | filename + embedding 检查，具体阈值 `[MISSING]` | 重叠数/率 | `[CODE][OBSERVED]` DATA-05 已确认 archive/UIQ 的 848-ID 精确相等；本地固定 metadata 得 4 个同源视频候选，但非音频重复结论 | DATA-08/09 后运行 canonical join；论文 847 排除 ID、完整 WavCaps 音频、论文嵌入模型/阈值仍 `[MISSING]` | C | IN_PROGRESS |
