...
| FIG-02 | 图 2；§3.1；附录 A | 架构核验：共享 backbone、LoRA、双投影头、512 维、L2 | 5 个附带 Clotho 样例可做 smoke | 三种 OEA backbone | 结构与参数形状 | `[CODE]` 锁绑定生成器严格加载 LoRA/双 head；`[OBSERVED]` Qwen3B、Nemo3B、Qwen7B 均完成独立官方源码 smoke 并输出 512 维 | `results/audits/fig02_architecture_verification_20260728.json` 记录方法契约；Qwen7B runtime 由 2026-07-30 两个正式 run 补充验证 | B（方法/张量契约已核验） | COMPLETED |
| FIG-03 | 图 3；§3.3；附录 L | 否定查询指标示意与显式配对评测 | 合成排名定义与真实 embedding 运行 | 与模型无关 | R@k、HNSR、HNSR@k、TFR、TFR-HN@k、Δ-Rank | `[CODE]` `AudioRetrieval/evaluation/negative_metrics.py`、`negative_canonical.py`；`scripts/run_negative_embedding_evaluation.sh` | 无 GPU；固定 embedding 与 pairing metadata | A（公式、显式配对评测与运行证据完整） | COMPLETED |
...
| EXP-06 | 附录 B.1；表 6 | 数据来源与潜在污染关系审计 | WavCaps 子集与 7 个评测集 | 文件/来源匹配 | 来源对应关系 | `[CODE][OBSERVED]` DATA-05 已固定 MECAT 848-row manifest；远程 DATA-08/09 已完成 AudioCaps/Clotho metadata provenance，DATA-11 已完成 MECAT source-video canonical join | 论文表 6 其余评测集关系、exact blocklist/final manifest 仍 `[MISSING]` | B | IN_PROGRESS |
...
| EXP-09 | §4.1；附录 B.4 | MECAT–WavCaps 无显著重叠核验 | MECAT 与 WavCaps | filename + embedding 检查，具体阈值 `[MISSING]` | 重叠数/率 | `[CODE][OBSERVED]` DATA-11 canonical join 在 848-row 公共集与固定 WavCaps metadata 间得到 4 个同源视频候选，但非音频重复结论 | 论文 847 排除 ID、完整 WavCaps 音频、论文嵌入模型/阈值与人工复核候选仍 `[MISSING]` | C | IN_PROGRESS |
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（公开归档为 848） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | `[OBSERVED]` 三数据集 13 个模型的 public controlled 默认与 sensitivity 协议均已执行，覆盖 `39/39` 模型×数据集单元；MECAT 固定 public-848 `short_all`，不做事后排除或协议择优 | MECAT 847-row 排除 ID、论文 caption selection 与音频 `passage:` 行为仍 `[MISSING]` | C（public controlled 矩阵完成，strict 未完成） | IN_PROGRESS |
| EXP-11 | §5.2；表 3 | caption Text-to-Text 完整基线 | 同 EXP-10 captions/candidates | 同 EXP-10 | R@1/5/10 | `[OBSERVED]` 三数据集 13 个模型的 public controlled seed0 与 all-caption sensitivity 均已执行，覆盖 `39/39` 模型×数据集单元；全部预声明协议均保留 | MECAT 847-row 排除 ID、论文 caption selection、self-exclusion 与 tie 口径仍 `[MISSING]` | C（public controlled 矩阵完成，strict 未完成） | IN_PROGRESS |
| EXP-12 | 附录 G.1；表 12 | Question UIQ T2A | AudioCaps/Clotho/MECAT-UIQ | 4 CLAP + 6 OEA | 每数据集 R@1/5/10 | `[OBSERVED]` 三数据集 10 个模型的 released Question UIQ public controlled 评测已执行，覆盖 `30/30` 单元 | 严格 baseline UIQ 完整命令未发布；MECAT PAPER 847/public 848 | B（public controlled 矩阵完成，strict 未完成） | IN_PROGRESS |
| EXP-13 | 附录 G.2；表 13 | Imperative UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | `[OBSERVED]` 三数据集 10 个模型的 released Imperative UIQ public controlled 评测已执行，覆盖 `30/30` 单元 | 同 EXP-12 | B（public controlled 矩阵完成，strict 未完成） | IN_PROGRESS |
| EXP-14 | 附录 G.3；表 14 | Paraphrase UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | `[OBSERVED]` 三数据集 10 个模型的 released Paraphrase UIQ public controlled 评测已执行，覆盖 `30/30` 单元 | 同 EXP-12 | B（public controlled 矩阵完成，strict 未完成） | IN_PROGRESS |
| EXP-15 | 附录 G.4；表 15 | Keyphrase/`tagging` UIQ T2A | 同 EXP-12 | 同 EXP-12 | 每数据集 R@1/5/10 | `[OBSERVED]` 三数据集 10 个模型的 released `tagging` UIQ public controlled 评测已执行，覆盖 `30/30` 单元；保留 `[CODE] tagging` 与 `[PAPER] Keyphrase` 双标签 | 同 EXP-12 | B（public controlled 矩阵完成，strict 未完成） | IN_PROGRESS |
| EXP-16 | §5.3.2；表 17 | Negative UIQ 普通检索 | 630/542/409 条 negative rows | 4 CLAP + 6 OEA | mean R@5、R@10 | `[CODE][OBSERVED]` 三数据集 deterministic caption-identity 配对审计已达到 `1,581/1,581`；M2D-CLAP 三数据集 controlled 评测已完成并生成等权均值，其余九模型待运行 | strict PAPER target/HN 映射仍未发布；controlled 模型结果进行中 | C | IN_PROGRESS |
| EXP-17 | §3.3/§5.3.2；附录 L/M；表 4、17 | Hard-negative discrimination | 同 EXP-16 + target/HN 音频 ID | 4 CLAP + 6 OEA | Δ-Rank、HNSR、HNSR@10、TFR、TFR-HN@10 | `[CODE][OBSERVED]` deterministic released-caption identity 在三数据集达到 `1,581/1,581`；M2D-CLAP Table 17 controlled mean 已完成，最大论文差为 HNSR@10 的 `5.0564 pp`；全量排名/哈希已固定 | 配对为 `[INFERRED]` controlled evidence；strict PAPER 显式 HN ID 仍未发布；其余九模型待运行 | C | IN_PROGRESS |
| EXP-18 | §5.4；附录 H；表 5、16 | 推理效率 | Clotho 1,045 clips | LAION/MGA/M2D + 3 OEA | audio ms/clip、text ms/query、peak GPU GB | `[OBSERVED]` Qwen3B(+Cl) 已完成 A100/RTX 4090；Nemo3B(+Cl)、M2D、MGA 已完成 RTX 4090 controlled benchmark；`[MISSING]` 论文 batch/warmup/timing/memory 口径，不能提升为严格表 5/16 复现 | 已固定四模型受控结果；仍需 LAION/Qwen7B，固定论文协议继续 `[MISSING]` | C（controlled-only） | IN_PROGRESS |
| EXP-19 | §3.1/§5.4；表 5、16 | 训练参数量/占比 | 不依赖数据 | 3 OEA + 3 CLAP | trainable params、占总参数百分比 | `[CHECKPOINT][OBSERVED]` Qwen3B(+Cl)/Nemo3B(+Cl)=14.71488M；M2D all/requires-grad=`89.041922/89.041920M`；MGA=`148.092376/147.008920M`；`[MISSING]` 论文统计口径 | 已固定四模型 checkpoint 实测分栏；仍需 LAION/Qwen7B，不能按最接近论文值选择统计口径 | B（controlled-only） | IN_PROGRESS |
...
| DER-01 | 附录 F；表 11 | 三数据集 T2A/T2T 均值汇总 | EXP-10、EXP-11 | 所有基线/OEA | mean R@1/5/10 | `[CODE][OBSERVED]` `scripts/build_table11_public_controlled_means.py` 从固定审计工件生成 13 模型×2 任务×3 指标；输出 `results/audits/table11_public_controlled_means_eval_20260803.json` 与 CSV | controlled 输入齐全；严格输入继承 EXP-10/11 的 MECAT 847/caption/self/tie 缺口 | B（controlled 派生完成，strict 未完成） | IN_PROGRESS |
