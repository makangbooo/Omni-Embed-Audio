...
| EXP-10 | §5.1；表 2 | caption Text-to-Audio 完整基线 | AudioCaps 975、Clotho 1,045、MECAT 847（发布 UIQ 为 848，需澄清） | 4 CLAP + 3 vanilla LALM + 6 OEA | R@1/5/10 | 六个 OEA 变体及全部三个 vanilla LALM 在 Clotho 上完成全量 embedding 和双协议 CPU 闭环；Qwen7B all-caption=`0.0957/0.6507/1.2440`、seed0=`0.0000/0.6699/1.3397`、论文=`0.10/0.65/1.32`；base-only loader/官方 metric source 且未加载 OEA checkpoint/LoRA/head | AudioCaps 实际音频、MECAT 847/caption 口径、四个 CLAP baseline 和论文 caption 选择仍缺失 | C（九个模型的 Clotho 分栏闭环完成） | IN_PROGRESS |
| EXP-11 | §5.2；表 3 | caption Text-to-Text 完整基线 | 同 EXP-10 captions/candidates | 同 EXP-10 | R@1/5/10 | 六个 OEA/Clotho 变体及全部三个 vanilla LALM 的 `[CODE]` seed0 与 `[INFERRED]` all-caption 均已分栏；Qwen7B seed0=`40.7656/54.1627/59.8086`、all-caption=`39.4067/52.9761/59.1005`，论文=`40.23/54.26/60.27`；未按接近度择优 | 其余模型/数据；论文 caption 选择、self-exclusion、tie 口径 `[MISSING]` | C（九个模型的 Clotho 分栏闭环完成） | IN_PROGRESS |
