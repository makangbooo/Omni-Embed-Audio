...
| EXP-06 | 附录 B.1；表 6 | 数据来源与潜在污染关系审计 | WavCaps 子集与 7 个评测集 | 文件/来源匹配 | 来源对应关系 | `[CODE][OBSERVED]` DATA-05 已固定 MECAT 848-row manifest；远程 DATA-08/09 已完成 AudioCaps/Clotho metadata provenance 复算 | DATA-11 canonical MECAT join；论文 exact blocklist/final manifest 仍 `[MISSING]` | B | IN_PROGRESS |
...
| EXP-09 | §4.1；附录 B.4 | MECAT–WavCaps 无显著重叠核验 | MECAT 与 WavCaps | filename + embedding 检查，具体阈值 `[MISSING]` | 重叠数/率 | `[CODE][OBSERVED]` DATA-05 与 DATA-08/09 已完成；本地固定 metadata 得 4 个同源视频候选，但非音频重复结论 | 运行 DATA-11 canonical join；论文 847 排除 ID、完整 WavCaps 音频、论文嵌入模型/阈值仍 `[MISSING]` | C | IN_PROGRESS |
