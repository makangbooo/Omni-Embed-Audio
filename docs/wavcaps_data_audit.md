...
The full local validation was run twice; the second pass reused the outputs only after verifying deterministic hashes. Remote DATA-08/09 has now completed on the shared CPU storage with all four child/wrapper exit codes equal to zero. Its evidence JSON, eight source identities, and five generated-artifact identities are fixed in `results/audits/data08_data09_wavcaps_remote_20260729.json` and match the prior local full audit. DATA-11 subsequently joined the canonical MECAT manifest to the pinned AudioSet_SL metadata; its four-candidate artifact is recorded separately from every training blocklist in `results/audits/data11_mecat_wavcaps_remote_20260729.json`.
