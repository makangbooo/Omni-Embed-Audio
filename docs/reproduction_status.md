...
| 0 编排 | 统一复现入口与 CPU/GPU 阶段门禁 | COMPLETED | `35ea0bd` | 公共入口、精确阶段注册表、默认只规划/显式执行、长任务确认和干净 worktree 门禁已实现；baseline 计划已展开为 vanilla CPU 模型锁 → A100 smoke → A100 full → CPU metrics 与独立 CLAP blocker；全量 223 项测试通过；尚未在远程执行这些新阶段 | 无；入口完成不代表任何 blocked 实验已解锁或产生复现值 | 仅在固定资源与证据满足后更新叶子阶段状态；当前仍只等待 DATA-04 |
...
| 5 基线 | 四个 CLAP 与三个 vanilla backbone 静态就绪度审计 | COMPLETED | `2583e2b` | 干净 `2583e2b` 上生成 `results/audits/baseline_readiness_2583e2b.json`：7 个模型、5 个代码入口完整、3 个资源身份固定、0 个正式可运行、0 证据漂移；三种 vanilla 已具备固定协议、模型锁解析、base-hidden-dimension 可恢复生成器及强制 5-audio/25-query smoke→full 闸门；真实锁和 GPU smoke 均尚未运行；完整 221 项测试通过 | 无；代码/锁工具完成不代表基线已评测 | DATA-04 后按长任务规则逐个生成并提交小型 vanilla base 锁，再申请 1×A100-80GB 运行锁绑定 smoke；四个 CLAP 仍需固定 source/checkpoint |
