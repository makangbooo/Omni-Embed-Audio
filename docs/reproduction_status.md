...
| 2 官方权重 | 正式评测协议与可移植模型锁强绑定 | COMPLETED | `d7ffb10` | Caption/UIQ GPU 生成器、CPU 套件及 5/25 smoke 均强制解析已提交模型锁；三个正式 Qwen3B wrapper 使用不可绕过的单 CUDA GPU + BF16 能力门禁并记录型号/显存到 `gpu_preflight.json`；4090 与 A100 均被单元覆盖，完整 233 项测试通过 | 无；Qwen3B 正确性评测不再把 A100 误写成必要条件 | 当前 RTX 4090 拉取最新 commit 后用新 RUN_ID 重跑一次，以生成当前能力门禁下的正式 smoke 证据 |
...
| 2 官方权重 | Qwen3B-Cl 正式生成器锁绑定 5/25 GPU smoke | WAITING_USER | `d7ffb10` | RTX 4090 运行 `...lock_bound_smoke_seed42_20260719_172349` 已完成 5×512/25×512、峰值 allocated 9.14 GiB、exit 0，证明当前 3B retrieval 路径在 24GB 上可运行；旧外层 A100 断言与继续执行的矛盾证据已保留 | 需要在最新能力门禁 commit 上重跑，补齐 `gpu_preflight.json` 后才能闭合正式 smoke | 保持当前 4090 实例，拉取最新分支并用新 RUN_ID 重跑；回传 preflight、metrics 与 attempt 日志 |
| 2 官方权重 | Tables 2/3：单个 3B 官方权重 T2A/T2T | WAITING_USER | `d7ffb10` | Clotho 1,045 条 evaluation manifest、Qwen3B-Cl 正式模型锁及 4090 实测显存均已准备；GPU 全量 embedding 尚未开始 | 等待最新能力门禁下的 5/25 smoke；完整 AudioCaps/MECAT 候选集仍未准备，因此当前只运行 Clotho 闭环 | smoke 通过后单独汇报 4090 全量命令、显存、时间、磁盘和输出目录，再等待确认 |
...
| 2 官方权重 | Qwen3B-Cl/Clotho T2A/T2T 四协议 CPU 评测套件 | COMPLETED | `2a3a1bd` | default-joint T2A all captions、T2A-only seed0、default T2T seed0、T2T all-caption sensitivity 已分栏；生成工件/协议/排名/日志/commit/exit 全量复核和非覆盖失败证据已实现；完整 122 项测试通过 | 远程正式 embedding 尚未生成；论文未公开 caption 选择，四协议不得择优冒充论文口径 | GPU embedding 完成后在 CPU 服务器运行套件并回传 `suite_metrics.json`/CSV |
...
| 2 结果管理 | 论文值/复现观测/证据统一汇总契约 | COMPLETED | `0c98b08` | Tables 1–17 已全部分类；13 张数值结果表共 910 个指标完成 PDF 页码/SHA256 固定与逐行转录审计（910/910 匹配）；6 个真实 blocked 观测进入 CSV，904 个待运行指标保持 unobserved；完整 152 项测试通过 | 无；工具完成不代表待运行指标已有复现值 | 正式实验完成后仅凭固定 JSON 证据哈希追加 observation，不自动选择协议或判定 close |
...
| 2 UIQ | Tables 12–15 正向 UIQ | IN_PROGRESS | `d7ffb10` | Clotho 四类固定源文件、4,180-query 可恢复单卡生成器与四协议 CPU canonical-ID 套件已实现；Qwen3B wrapper 已改为单 GPU + BF16 能力门禁，4090 可用于正确性评测 | 正式 caption/UIQ embedding 尚未生成；AudioCaps/MECAT 音频候选集未准备；MECAT 847/848 口径未决 | 完成 caption 全量后可继续在同一 4090 生成 Clotho UIQ embedding，再回到 CPU 运行 Tables 12–15 套件 |
...
- 当前阶段：Qwen3B-Cl/Clotho 官方权重评测准备已完成到模型锁和 4090 实测；正在等待当前 4090 在最新能力门禁 commit 上重跑 5-audio/25-caption smoke。
- 当前对应论文范围：所有主表 1–5、附录表 6–17、图 1–3、附录 A–M 已建清单。
- 最近完成：Qwen3B-Cl 正式模型锁已提交；RTX 4090 真实运行确认正式生成器使用 GPU 完成 5/25 embedding，峰值 allocated 9.14 GiB；据此撤销无依据的 A100-80GB 正确性评测强制条件，改为单 CUDA GPU + BF16 能力门禁并继续记录型号/显存。A100-SXM4-80GB 仅保留给论文 Table 5/16 绝对效率硬件对齐；完整 233 项测试通过。
- 当前阻塞：全量生成前只缺最新能力门禁下的 4090 锁绑定 smoke 证据；无需更换服务器。正式论文全三数据集表仍缺 AudioCaps 与 MECAT 音频及若干论文未公开口径，这些不阻塞当前 Clotho smoke。
- 下一步：当前 4090 服务器先检查 worktree 并拉取最新 `repro/oea-full`，用新 RUN_ID 重跑 5/25 smoke。收到 `gpu_preflight.json` 与完整 metrics 后，再决定是否授权 1,045 audio/5,225 caption 全量生成。
