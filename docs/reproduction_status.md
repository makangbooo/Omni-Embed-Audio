...
| 3 数据 | DATA-02：Clotho evaluation 解压与完整性校验 | WAITING_USER | `e938cb5` | 等待 CPU 服务器检查 7z 并运行 | 尚未确认服务器是否已有 `7zz`/`7z`/`7za` | 拉取最新 commit，执行只读预检；有 extractor 后运行完整校验 |
...
- 下一步：在 CPU 服务器执行 DATA-02 预检与完整校验，同时等待 MODEL-02 下载结果；校验成功后归档小型统计并开始 canonical Clotho evaluator。
