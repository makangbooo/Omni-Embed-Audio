...
| 1 环境 | CPU-03：安装 DATA-02 的 7-Zip 工具 | BLOCKED | `99b88a3` | MODEL-02 正在使用共享 `oea-repro` | 不能在活跃模型下载期间执行 Conda transaction | MODEL-02 结束后安装固定 `7zip=26.02` |
...
| 2 官方权重 | MODEL-02：OEA-Qwen3B-AC checkpoint | RUNNING_REMOTE | `30f003a` | 进程 39554；运行目录 `logs/model02_download_20260716_150343` | 正在下载固定 revision 的 `step_350.pt` | 监控 `.incomplete` 增长；完成后核对 manifest 和 LFS SHA256 |
...
- 下一步：等待 MODEL-02 下载进程结束并核对 LFS SHA256；随后执行 CPU-03 固定 7-Zip 安装，再运行 DATA-02。
