...
| 2 官方权重 | 正式评测协议与可移植模型锁强绑定 | COMPLETED | `0ab2daa` | Caption/UIQ GPU 生成器、CPU 套件及新增的 5-audio/25-caption smoke 均强制解析已提交模型锁；smoke 与全量使用同一生成器、模型配置、公开代码 prompt 协议和 batch size；完整 230 项测试通过 | 真实锁绑定 smoke 尚未在 GPU 执行 | 切换到 1×A100-80GB，先运行 `run_qwen3b_cl_clotho_lock_bound_smoke.sh`；只有报告 complete 后才解除全量门禁 |
...
| 2 官方权重 | Qwen3B-Cl 正式生成器锁绑定 5/25 GPU smoke | WAITING_USER | `0ab2daa` | 固定 fixture、锁解析、完整资源 SHA256、严格离线、可恢复 chunk、输出审计与独立失败证据入口均已提交；尚未远程运行 | 需要用户把共享 `/home/jg525` 挂载到 1×A100-80GB 实例 | 拉取最新 commit，完成 GPU/共享挂载预检后运行短 smoke，并回传 `generation_metrics.json` 与 attempt 日志 |
...
- 最近完成：Clotho DATA-02 远程全量解码/manifest/UIQ 校验通过；WavCaps DATA-09 精确复现论文的 173 个 AudioCaps 和 638 个 Clotho 重叠计数；Qwen3B-Cl 三阶段 CPU 流水线已验证完整 base、官方原始 checkpoint 与既有 inference-only 权重，真实模型锁已按原始字节提交；正式生成器新增同锁、同模型配置、同公开代码 prompt 协议的 5 音频/25 caption 独立 smoke 门禁；其余 checkpoint、数据、UIQ、负查询、基线与论文结果 inventory 进度保持不变；完整 230 项测试通过。
