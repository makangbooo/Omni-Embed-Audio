...
| 1 环境 | CPU-03：安装 DATA-02 的 7-Zip 工具 | WAITING_USER | `99b88a3` | 预检确认 `7zz`/`7z`/`7za` 均不存在 | DATA-02 无可用 extractor | 确认无 OEA 作业占用共享环境后，从清华 conda-forge 镜像安装固定 `7zip=26.02` |
...
- 下一步：先在 CPU 服务器完成 CPU-03 固定 7-Zip 工具安装，再运行 DATA-02；同时等待 MODEL-02 下载结果。
