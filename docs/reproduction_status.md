...
| 2 官方权重 | 正式评测协议与可移植模型锁强绑定 | COMPLETED | `5fc1606` | Caption/UIQ GPU 生成器、CPU 套件及 5/25 smoke 均强制解析已提交模型锁；三个正式 GPU wrapper 又增加不可绕过的单卡 A100、≥79 GiB、BF16 fail-closed 门禁及 `gpu_preflight.json` 证据；完整 234 项测试通过 | 无；门禁实现完成不代表 A100 smoke 已运行 | 切换到 1×A100-80GB，拉取修复后重新运行锁绑定 smoke；只有 A100 报告 complete 后才解除全量门禁 |
...
| 2 官方权重 | Qwen3B-Cl 正式生成器锁绑定 5/25 GPU smoke | WAITING_USER | `5fc1606` | 错误硬件运行 `...lock_bound_smoke_seed42_20260719_172349` 在 RTX 4090 上完成 5×512/25×512、峰值 allocated 9.14 GiB、exit 0；它证明 GPU 路径可用，但不计作计划中的 A100 smoke。外层预检返回码未阻止 wrapper 的缺陷已保存并修复 | 当前实例是 RTX 4090；需要更换到 1×A100-80GB | 更换服务器后拉取 `5fc1606` 及后续状态 commit，用新 RUN_ID 重跑；回传 `gpu_preflight.json`、metrics 与 attempt 日志 |
| 2 官方权重 | Tables 2/3：单个 3B 官方权重 T2A/T2T | WAITING_USER | `0ab2daa` | Clotho 1,045 条 evaluation manifest、Qwen3B-Cl 正式模型锁及锁绑定 smoke 入口均已准备；GPU 正式 embedding 尚未开始 | 等待 5/25 smoke 通过；完整 AudioCaps/MECAT 候选集仍未准备，因此当前只运行 Clotho 闭环 | smoke 通过后单独汇报全量命令、单卡显存、时间、磁盘和输出目录，再等待确认 |
...
- 当前阶段：Qwen3B-Cl/Clotho 官方权重评测准备已完成到正式模型锁提交；正在等待 1×A100-80GB 执行锁绑定 5-audio/25-caption smoke。
- 当前对应论文范围：所有主表 1–5、附录表 6–17、图 1–3、附录 A–M 已建清单。
- 最近完成：Qwen3B-Cl 正式模型锁已提交；RTX 4090 替代硬件运行确认正式生成器确实使用 GPU 并完成 5/25 embedding，峰值 allocated 9.14 GiB；该运行暴露的预检未 fail-closed 问题已用三个正式 wrapper 共用的单卡 A100-80GB 强门禁修复，错误硬件以后会在模型加载前退出并留下 `gpu_preflight.json`；其余 checkpoint、数据、UIQ、负查询、基线与论文结果 inventory 进度保持不变；完整 234 项测试通过。
- 当前阻塞：当前可执行的 Clotho 闭环只缺 1×A100-80GB 上的锁绑定 smoke；全量生成在 smoke 完成前保持 BLOCKED。正式论文全三数据集表仍缺 AudioCaps 与 MECAT 音频及若干论文未公开口径，这些不阻塞当前 Clotho smoke。
- 下一步：把共享 `/home/jg525` 挂载到 1×A100-80GB 实例，先检查远程 worktree，再拉取 `repro/oea-full` 最新 commit；完成 GPU、模型锁、checkpoint 和 fixture 预检后运行 5/25 smoke。收到完整报告后再决定是否授权 1,045 audio/5,225 caption 全量生成。
