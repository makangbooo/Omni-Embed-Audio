...
| 5 基线 | 四个 CLAP 与三个 vanilla backbone 静态就绪度审计 | COMPLETED | `da84676` | 干净 commit 上生成 `results/audits/baseline_readiness_da84676.json`：7 个模型、2 个代码入口完整、3 个资源身份固定、0 个正式可运行、0 证据漂移；完整 202 项测试通过 | 无；审计完成不代表基线已评测 | 共同候选集完成后逐个固定四个 CLAP 的 source/checkpoint；vanilla 先补 base-only 模型锁和非覆盖 embedding wrapper |
