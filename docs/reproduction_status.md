...
| 2 官方权重 | 官方模型 CPU 三阶段流水线与已有派生权重只读复核 | COMPLETED | `6b3aa1b` | Qwen3B-Cl 修复后真实远程运行 `logs/official_model_pipeline_oea_qwen3b_cl_20260719_163246` 三阶段 exit 均为 0：base 18/18、原始 checkpoint 3/3、注册表唯一派生路径及既有派生权重逐张量复核全部通过；生成 5,292-byte 模型锁，SHA256 `790d12ccc8f25ea29eecb37519db5c24262253f85dcb9055b473a45304589e36` | 无；首次失败证据 `..._145123` 仍保留，修复不修改任何原始/派生权重或评测参数 | 提交并推送小型模型锁；GPU 服务器拉取锁 commit 后执行 1×A100-80GB 正式 embedding 小批校验 |
| 2 官方权重 | 正式评测协议与可移植模型锁强绑定 | COMPLETED | `6b3aa1b` | Caption/UIQ GPU 生成器及其 CPU 评测套件均要求已提交的模型锁；Qwen3B-Cl 真实模型锁已由三阶段 CPU 流水线生成并通过本地字节级复核：5,292 bytes、SHA256 `790d12ccc8f25ea29eecb37519db5c24262253f85dcb9055b473a45304589e36` | 等待模型锁 commit 被 GPU 服务器拉取；尚未对正式 GPU embedding 运行绑定入口 | 提交模型锁后申请 1×A100-80GB，先执行锁绑定小批校验，再决定是否启动 1,045 audio/5,225 caption 全量生成 |
...
| 2 官方权重 | Tables 2/3：单个 3B 官方权重 T2A/T2T | WAITING_USER | `6b3aa1b` | Clotho 1,045 条 evaluation manifest 与 Qwen3B-Cl 正式模型锁均已完成真实远程审计；GPU 正式 embedding 尚未开始 | 需要切换到 1×A100-80GB；完整 AudioCaps/MECAT 候选集仍未准备，因此当前只运行 Clotho 闭环 | 模型锁 commit/push 后在 A100 服务器先执行固定小批校验，通过后再生成全量 audio/caption embedding |
...
- 最近完成：Clotho DATA-02 远程全量解码/manifest/UIQ 校验通过；WavCaps DATA-09 精确复现论文的 173 个 AudioCaps 和 638 个 Clotho 重叠计数；DATA-10 完成 development/validation 四个 CSV 全量审计与远程全量校验实现；MODEL-03/04 只读完整性审计器及六变体按变体精确资源审计入口完成；六个 OEA 原始 checkpoint 的真实文件名、revision、字节数与 LFS SHA256 已固定，并完成清单驱动的 FakeTensor 结构审计/非覆盖推理权重提取器；六变体可移植模型资源锁生成器已把基础模型全文件、原始/派生 checkpoint、实测 LoRA 结构及证据报告纳入同一校验契约；进一步完成固定顺序的三阶段 CPU 流水线、已有派生权重逐张量只读复核，以及正式 Caption/UIQ GPU 生成和 CPU 汇总对已提交模型锁/完整 base 文件清单的双端强绑定；DATA-11 在固定元数据上发现 4 个 MECAT 同源视频候选；显式 target/HN negative artifact evaluator、Qwen3B-Cl/Clotho 四协议 caption retrieval 套件，以及 Tables 12–15 正向 UIQ 的 4,180-query 可恢复生成器/CPU 套件完成；Tables 1–17 全部分类，13 张数值结果表的 910 个论文指标已登记并通过固定 PDF 的 910/910 单元转录审计；四个 CLAP 与三个 vanilla backbone 的机器可校验就绪度审计现为 5/7 代码入口完整、3/7 资源身份固定、0/7 正式可运行；三种 vanilla 的 base-only 资源锁流水线、固定 Clotho 协议、锁绑定可恢复 embedding 生成器、不可绕过的 smoke→full 闸门和锁/协议 SHA/hidden dimension/no-OEA-flags 强校验 CPU 四协议 finalizer 均已实现，并把论文 `passage:` 与公开代码 audio-only no-prefix 冲突保留为显式协议字段；统一入口已把 vanilla CPU 模型锁、A100 smoke、A100 full 和 CPU finalizer 分成四个不可误触叶子阶段，CLAP blockers 单独保留；完整 227 项测试通过。
