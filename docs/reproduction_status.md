...
| 2 OEA-4 A2T | Qwen3B-Cl/Clotho 1,045 音频查询→5,225 冻结 caption 候选 | WAITING_USER | `df3a532` | `[CODE]` 五正例 A2T canonical evaluator、锁绑定 CPU suite、完整排名/哈希/失败保留和 254 项全量回归均已完成并 push；正式指标尚未运行 | 需要用户将共享目录挂载到 CPU 服务器并执行已提交的 CPU-only suite | CPU 服务器拉取 `repro/oea-full@7207831` 后复用固定 embedding 运行，回传 suite metrics、CSV、哈希、日志尾部与 clean status |
...
| 2 OEA-6 效率 | OEA-Qwen3B-Cl 查询编码延迟、显存、吞吐与参数量 | IN_PROGRESS | `d156862` | 已固定 Table 5/16 论文值并实现锁绑定、单卡 A100、BF16、batch=1、warmup=10、Clotho 全量音频/文本、CUDA 同步、逐项 latency 与 mean/std/P50/P95/吞吐/显存/参数量 benchmark | `[MISSING]` 论文未公开 warmup、重复次数、batch、cache 和 timing scope；正式 A100 运行尚未执行 | 在 1×A100-SXM4-80GB 服务器运行，保留与论文口径分栏的 `[INFERRED]` 测量协议 |
