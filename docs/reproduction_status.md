...
| 5 基线 | 四个 CLAP 与三个 vanilla backbone 静态就绪度审计 | COMPLETED | `5a40768` | 干净 `5a40768` 上生成 `results/audits/baseline_readiness_5a40768.json`：7 个模型、2 个代码入口完整、3 个资源身份固定、0 个正式可运行、0 证据漂移；三种 vanilla 已新增“固定 registry → 逐文件只读审计 → 非覆盖 base-only 锁”CPU 流水线，但真实锁尚未远程生成；完整 210 项测试通过 | 无；审计/锁工具完成不代表基线已评测 | DATA-04 后按长任务规则逐个生成并提交小型 vanilla base 锁；共同候选集完成后再实现锁绑定 embedding wrapper；四个 CLAP 仍需固定 source/checkpoint |
