...
| 2 官方权重 | MODEL-03：Nemotron-3B base + OEA-Nemo3B AC/Cl | RUNNING_REMOTE | `ae2c3fb` | PID 50168；`logs/model03_download_20260716_171115`；有效分片约 734 MB/975 MB 且持续更新 | CAS 间歇返回 504，但官方/镜像 range test 均为 HTTP 206 | 保持运行；完成后按 manifest 校验全部 LFS SHA256 |
| 2 官方权重 | MODEL-04：Qwen2.5-Omni-7B base + OEA-Qwen7B AC/Cl | RUNNING_REMOTE | `ae2c3fb` | PID 48944；`logs/model04_download_20260716_170735`；5 个分片持续更新 | 启动时 `git_status.txt` 捕获两个瞬态 DPC 临时文件；统一快照时工作区已干净 | 保持运行；下载后重新以干净工作区做资源校验 |
...
- 下一步：保持四个 tmux 下载任务运行；不增加 CAS 并发、不修改共享环境；任一任务结束后立即核对 manifest、exit code、残留 partial 与 checksum。
