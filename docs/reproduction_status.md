...
| 2 OEA-4 A2T | Qwen3B-Cl/Clotho 1,045 音频查询→5,225 冻结 caption 候选 | COMPLETED | `b4986d3` | CPU suite `...a2t_suite_seed42_20260721_201518` 在 clean worktree 完成：status=complete、protocol_count=1、exit 0、stderr 为空；R@1/5/10=`27.2727/52.8230/66.6986`，MRR=`0.395682`，DCG=`0.517412`；CSV/Suite SHA256 已固定 | 无；该任务是未见于论文表格的 `[CODE]` 官方 checkpoint 扩展，不能标成论文数值复现；额外预检误读不存在的根目录 `metrics.json`，但不影响套件的独立完整性校验与结果 | 小型审计证据已登记；下一步在 1×A100-SXM4-80GB 上执行 OEA-6 正式效率评测 |
...
| 2 OEA-6 效率 | OEA-Qwen3B-Cl 查询编码延迟、显存、吞吐与参数量 | WAITING_USER | `d156862` | 已固定 Table 5/16 论文值并实现锁绑定、单卡 A100、BF16、batch=1、warmup=10、Clotho 全量音频/文本、CUDA 同步、逐项 latency 与 mean/std/P50/P95/吞吐/显存/参数量 benchmark；254 项全量回归通过 | `[MISSING]` 论文未公开 warmup、重复次数、batch、cache 和 timing scope；需要用户将共享目录挂载到 1×A100-SXM4-80GB 服务器并启动正式评测 | 拉取 `repro/oea-full` 最新 commit 后运行 CPU→A100 的锁绑定 benchmark；完成后回传 metrics、哈希、exit、stderr 和 clean status |
