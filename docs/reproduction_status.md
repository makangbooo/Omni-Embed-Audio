...
| 2 官方权重 | 5 个 Clotho 样例 Qwen3B-Cl smoke | WAITING_USER | `66d87f9` | `FakeTensorMode` 审计完成；确认 checkpoint 含 9.41 GB 冻结 base、50.46 MB LoRA 和 8.40 MB 双 head | 等待生成并校验 inference-only 派生 checkpoint | CPU 提取约 59 MB 增量权重后做单卡 GPU 最小加载 |
...
- 当前阻塞：官方 9.47 GB checkpoint 把 4.70B 冻结 backbone 参数与 14.71M 可训练增量参数一起保存，直接 GPU 加载会产生不必要峰值。
- 下一步：用户拉取 `66d87f9`，在 CPU 上安全提取约 58.86 MB LoRA+双 head，逐 tensor 验证并生成独立 SHA256 后，再做单卡 GPU smoke。
