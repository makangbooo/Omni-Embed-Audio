...
| Phase-2 NO-GO diagnostic | CPU-only Whisper transcript/WER, N-best diversity, cross-condition agreement, OEA/vanilla embedding geometry, hubness, and positive-score characterization | `scripts/diagnose_asrur_phase2_no_go.py`, `tests/test_diagnose_asrur_phase2_no_go.py` | IMPLEMENTED; reads immutable completed caches only and does not authorize changing checkpoint, ASR protocol, or candidate generation |
