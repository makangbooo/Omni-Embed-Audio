...
| Phase-2 Whisper base-generation repair | Explicit English/transcribe/no-timestamps prompt plus the frozen model's base `GenerationMixin` four-beam path; same checkpoint and proxy-score definition | `AudioRetrieval/asr_uncertainty_reranking/model_adapters.py`, `tests/test_asrur_model_adapters.py` | WAITING_USER; 15 targeted and 409 full-repository CPU tests pass; requires the same one-record RTX-4090 smoke |
...
最后更新：2026-07-27（Whisper dtype smoke exposed the Transformers 4.52.4 beam-index bug; minimal base-generation repair awaits one-record GPU validation）
