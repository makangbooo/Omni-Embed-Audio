...
| Phase-2 WER-stratified retrieval analysis | Per-query top-1 WER joined to the fixed B1 per-query nDCG/MRR/Recall evidence; fixed WER bins, JSON/CSV/JSONL outputs, input hashes, no parameter selection | `scripts/analyze_asrur_phase2_wer.py`, `scripts/run_asrur_phase2_wer_analysis.sh`, `tests/test_analyze_asrur_phase2_wer.py` | IMPLEMENTED_AND_TESTED after attempt 5; CPU-only foreground runner records elapsed seconds and refuses output reuse; remote formal analysis pending |
...
最后更新：2026-07-28（Whisper attempt 5 已完成；OEA-Cl 全量独立重跑已实现并等待执行）
