...
| 2 | Whisper 单记录 BF16/FP32 数值诊断 | WAITING_USER | `a7735e4` | 首次远程运行完成三条数值路径后，在严格 JSON 序列化阶段因原始 beam `sequence_score=NaN` 而失败；无 cache mutation/研究指标；失败审计已保存。修复将非有限序列分数表示为 `null` + `sequence_score_class`，并增加严格 JSON 回归测试 | 等待 1×RTX 4090 仅重跑同一记录 | 根据成功写出的逐 token 诊断选择最小模型侧补丁 |
