...
| 1 | Nemo base/+Cl 当前缓存实时只读复核 | WAITING_USER | `86107ca` | attempt 1 run=`asrur_nemo_phase1_audit_20260726_232452` exit=`1/1`；`HF_HUB_OFFLINE=1` 与固定 revision 元数据 inventory 冲突，两个 asset 均在本地内容校验前失败；未发现模型损坏证据 | 修复已提交，需要远程重新拉取；全哈希可能超过 30 分钟 | 拉取修复后重新运行同一 wrapper |
| 1 | Nemo inference-only checkpoint 与 model lock | WAITING_USER | `86107ca` | attempt 1 未创建派生权重或 lock；修复后仅允许固定 revision 元数据 API，请求不下载模型；内容仍在本地验证 | 依赖 rerun；正式仓库锁只能由本地 Codex 根据返回证据提交 | rerun portable evidence 验证通过后生成并提交 canonical model lock |
