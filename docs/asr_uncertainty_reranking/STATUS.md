...
最后更新：2026-07-27（Nemo 5/25 锁绑定 GPU smoke 已就绪）
...
| 1 | Nemo 5/25 OEA embedding smoke | WAITING_USER | `ff73287` | 锁绑定 smoke/full config 与 wrapper 已提交；39 项相关 CPU 测试通过；GPU 尚未启动 | 需要用户切换到 1×RTX 4090 24GB 并执行 G1 smoke | 拉取最新分支；严格离线运行 5 音频/25 caption，返回 metrics、显存和完整日志 |
