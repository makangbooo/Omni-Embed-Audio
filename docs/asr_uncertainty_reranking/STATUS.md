...
| 1 | Nemo base/+Cl 当前缓存实时只读复核 | RUNNING_REMOTE | `cd34cbc` | attempt 3 run=`asrur_nemo_phase1_audit_20260727_000937`，PID=`41000`；三个 Hub offline alias 均已清除，metadata-only/no-download 提示与干净执行 commit 已确认 | 正在执行固定 revision 清单与本地内容校验，预计 40–120 分钟 | 等待退出码与完整 resource audit |
| 1 | Nemo inference-only checkpoint 与 model lock | RUNNING_REMOTE | `cd34cbc` | attempt 3 已选择 `extract_new_derived` 原子路径；无 GPU、无模型下载、不覆盖源 checkpoint | 依赖当前审计与提取完成；正式仓库锁只能由本地 Codex 根据返回证据提交 | portable evidence 验证通过后提交 canonical model lock |
