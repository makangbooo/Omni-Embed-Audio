...
| 1 | Nemo base/+Cl 当前缓存实时只读复核 | RUNNING_REMOTE | `5bba16c` | rerun=`asrur_nemo_phase1_audit_20260726_235146`，PID=`39612`；修复提示、metadata-only policy 与原子提取模式均已确认；执行 worktree 干净 | 正在进行全文件本地内容校验，预计 40–120 分钟 | 等待退出码与完整 resource audit |
| 1 | Nemo inference-only checkpoint 与 model lock | RUNNING_REMOTE | `5bba16c` | 同一 rerun 已进入 `extract_new_derived` 路径；无 GPU、无模型下载、不覆盖源 checkpoint | 依赖当前审计与提取完成；正式仓库锁只能由本地 Codex 根据返回证据提交 | portable evidence 验证通过后生成并提交 canonical model lock |
