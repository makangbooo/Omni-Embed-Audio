...
| 2 | Phase-2 attempt 4 严格恢复与完成 | WAITING_USER | 本提交 | 新 runner 将校验并复用 14 个完整 cache，逐条严格校验并复制 attempt 3 的 647 个 snr_0 shards，只生成缺失记录；每次允许重试的数值失败均保存 traceback，首个严格有限结果才可写 shard | 等待 1×RTX 4090 执行；尚无正式指标 | 完成四条件排名、指标和 Go/No-Go 审计 |
