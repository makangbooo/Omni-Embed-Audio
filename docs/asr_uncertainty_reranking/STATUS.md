...
| 1 | Nemo inference-only checkpoint 与 model lock | RUNNING_REMOTE | `58f80bb` | attempt 4 run=`asrur_nemo_phase1_audit_20260727_004107`，PID=`43010`；两个 PosixPath 精确 safe-global 名称、干净执行 commit、metadata-only/no-download 与原子提取模式均已确认 | 正在重新校验资源并准备派生 checkpoint；预计 40–120 分钟 | 等待 checkpoint preparation、portable lock 与两个退出码 |
