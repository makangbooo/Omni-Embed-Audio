...
| 1 | Nemo inference-only checkpoint 与 model lock | WAITING_USER | `7bd4765` | attempt 3 preparation exit=`1`：仅因 `pathlib._local.PosixPath` 未在精确 safe-global 白名单；未生成派生权重或 lock；补丁仅允许两个 PosixPath 精确名称并拒绝其他 global | 需要拉取安全兼容补丁后运行 attempt 4 | attempt 4 从已通过资源继续审计、原子提取并生成 portable lock |
