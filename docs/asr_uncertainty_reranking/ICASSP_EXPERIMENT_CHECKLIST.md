...
| OEA-B3 | SQuTR-FiQA OEA direct audio-to-text retrieval for Clean/20/10/0 dB | Primary frozen first-stage baseline and candidate generator | WAITING_USER | Attempt 2 produced all four complete OEA embedding caches but failed before ranking finalization; first Whisper repair smoke exposed a Transformers 4.52.4 beam-index bug; minimal base-generation repair awaits one-record RTX 4090 validation |
...
|---|---|---|---|
| ASR-E1 | B1: Whisper 1-best + BGE dense retrieval, four FiQA conditions | WAITING_USER | FP32/BF16 input mismatch is fixed; first smoke then exposed Transformers 4.52.4 global beam-index misuse after score re-stacking; same-model base-generation repair awaits one-record GPU smoke |
