...
AUDIT_PATH = (
    REPOSITORY_ROOT
    / "results/audits/m2d_clap_mecat_public848_positive_uiq_eval_20260802.json"
)


class M2DClapMecatPositiveUiqEvidenceTests(unittest.TestCase):
    @classmethod
    ...
    def test_remote_run_completed_with_official_source_and_gpu(self) -> None:
        ...
        self.assertEqual(self.audit["run"]["elapsed_seconds"], 109)
        self.assertTrue(self.audit["run"]["gpu_used"])
        self.assertTrue(self.audit["official_source"]["oea_official_source_used"])
        self.assertEqual(self.audit["generation"]["audio_embedding_shape"], [848, 768])
    ...
    def test_twelve_metrics_and_deltas_are_fixed(self) -> None:
        ...
        self.assertEqual(max(maxima), 2.710377)
        self.assertTrue(
