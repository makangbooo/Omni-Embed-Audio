...
class VerifySqutrExtractionCompletionTest(unittest.TestCase):
    ...
    def test_recovery_wrapper_is_cpu_only_resumable_and_non_destructive(self) -> None:
        ...
        self.assertIn("schema=oea_data13c_lock_v1", text)
        self.assertIn("last_owner_host=", text)
        self.assertIn("last_owner_pid=", text)
        self.assertIn("last_owner_git_commit=", text)
        self.assertIn("last_owner_run_dir=", text)
        self.assertIn("legacy empty lock file", text)
