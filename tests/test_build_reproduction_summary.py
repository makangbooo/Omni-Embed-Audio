...
class BuildReproductionSummaryTests(unittest.TestCase):
    ...
    def test_committed_summary_matches_default_sources(self) -> None:
        ...
        self.assertEqual(len(close), 168)
        ...
        self.assertEqual(result["unobserved_paper_metric_count"], 794)
        self.assertEqual(result["status_counts"], {"blocked": 23, "close": 168})

        nemo_t2t = [
            row
            for row in rows
            if row["experiment"].startswith("oea_nemo3b_cl_clotho_t2t_")
        ]
        ...
        nemo_ac = [
            ...
            if row["experiment"].startswith(
                "official_source_oea_nemo3b_ac_clotho_"
            )
        ]
        self.assertEqual(len(nemo_ac), 24)
        self.assertEqual({row["status"] for row in nemo_ac}, {"close"})
        ...
        vanilla_nemotron = [
            ...
            for row in rows
            if row["experiment"].startswith(
                "official_source_vanilla_nemotron_3b_clotho_"
            )
        ]
        ...
        vanilla_qwen7b = [
            ...
            for row in rows
            if row["experiment"].startswith(
                "official_source_vanilla_qwen2_5_omni_7b_clotho_"
            )
        ]
        self.assertEqual(len(vanilla_qwen7b), 12)
        self.assertEqual({row["status"] for row in vanilla_qwen7b}, {"close"})
