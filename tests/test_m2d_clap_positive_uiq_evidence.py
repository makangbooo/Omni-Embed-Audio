...
class M2DClapPositiveUiqEvidenceTests(unittest.TestCase):
    @classmethod
    ...
    def test_remote_run_and_four_protocols_completed(self) -> None:
        ...
        self.assertEqual(self.audit["run"]["elapsed_seconds"], 75)
    ...
    def test_observed_values_and_paper_deltas_are_fixed(self) -> None:
        ...
        self.assertEqual(
            comparison["paraphrase_released_uiq"]
    ...
    def test_twelve_close_observations_bind_to_exact_audit(self) -> None:
        relevant = [
            row
            for row in self.observations
            if row["observation_id"].startswith("m2d_clap_clotho_uiq_")
        ]
