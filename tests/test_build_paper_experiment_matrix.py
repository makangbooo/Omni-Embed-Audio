...
class BuildPaperExperimentMatrixTests(unittest.TestCase):
    ...
    def test_mga_clap_clotho_positive_uiq_cells_are_reproduced(self) -> None:
        for inventory, table, task in (
            ("EXP-12", "Table 12", "UIQ Question T2A"),
            ("EXP-13", "Table 13", "UIQ Imperative T2A"),
            ("EXP-14", "Table 14", "UIQ Paraphrase T2A"),
            ...
            self.assertEqual(
                row["evidence"],
                "results/audits/mga_clap_clotho_positive_uiq_eval_20260731.json",
            )

    def test_m2d_clap_clotho_positive_uiq_cells_are_reproduced(self) -> None:
        for inventory, table, task in (
            ("EXP-12", "Table 12", "UIQ Question T2A"),
            ("EXP-13", "Table 13", "UIQ Imperative T2A"),
            ("EXP-14", "Table 14", "UIQ Paraphrase T2A"),
            ...
            key = (inventory, table, "M2D-CLAP", "Clotho", task)
            row = self.by_key[key]
            self.assertEqual(row["reproduction_status"], "REPRODUCED_CLOSE")
            self.assertEqual(row["observed_metric_count"], 3)
            self.assertEqual(
                row["evidence"],
                "results/audits/m2d_clap_clotho_positive_uiq_eval_20260802.json",
            )

    def test_laion_clap_clotho_positive_uiq_cells_are_reproduced(self) -> None:
        for inventory, table, task in (
            ("EXP-12", "Table 12", "UIQ Question T2A"),
            ("EXP-13", "Table 13", "UIQ Imperative T2A"),
            ("EXP-14", "Table 14", "UIQ Paraphrase T2A"),
            ...
            key = (inventory, table, "LAION-CLAP", "Clotho", task)
            row = self.by_key[key]
            self.assertEqual(row["reproduction_status"], "REPRODUCED_CLOSE")
            self.assertEqual(row["observed_metric_count"], 3)
            self.assertEqual(
