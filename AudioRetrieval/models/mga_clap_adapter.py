...
class MGAClapAdapter:
    ...
    def __init__(
        ...
        # Compatibility with checkpoints produced by older Transformers versions.
        # position_ids is a deterministic, non-trainable buffer regenerated by BERT.
        compat_key = "text_encoder.text_encoder.embeddings.position_ids"

        if compat_key in state_dict and compat_key not in self.model.state_dict():
            state_dict.pop(compat_key)
            print(f"[MGA-CLAP] Dropped compatibility buffer: {compat_key}")

        # Keep strict validation for every learned parameter.
        self.model.load_state_dict(state_dict, strict=True)
