...
BENCHMARK_CONFIG="${ROOT_DIR}/configs/eval/nemo3b_cl_clotho_efficiency_rtx4090.json"
PROTOCOL_CONFIG="${ROOT_DIR}/configs/eval/nemo3b_cl_clotho_embeddings.json"
MODEL_LOCK="${ROOT_DIR}/results/model_locks/oea_nemo3b_cl.json"
RUN_ID="${RUN_ID:-oea_nemo3b_clotho_rtx4090_efficiency_$(date +%Y%m%d_%H%M%S)}"
...
required = "NVIDIA GeForce RTX 4090"
observed = torch.cuda.get_device_name(0)
if observed != required:
    raise SystemExit(f"[ERROR] Required GPU {required!r}; observed {observed!r}")
PY
...
echo "[INFO] Nemo RTX 4090 efficiency benchmark completed: ${OUTPUT_DIR}/metrics.json"
