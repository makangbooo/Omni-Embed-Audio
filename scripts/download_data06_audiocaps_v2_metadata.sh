...
RUN_ID="data06_audiocaps_v2_metadata_download_$(date +%Y%m%d_%H%M%S)"
RUN_DIR="${ROOT_DIR}/logs/${RUN_ID}"
MANIFEST="${ROOT_DIR}/configs/resources/data06_audiocaps_v2_metadata.json"
...
du -sh "${DATA_ROOT}/audiocaps_v2_d004db3" \
  > "${RUN_DIR}/dataset_size.txt" 2>&1 || true
if [[ "${DOWNLOAD_EXIT_CODE}" -ne 0 ]]; then
  echo "[ERROR] DATA-06 download failed; partial files were preserved." >&2
  echo "[ERROR] Audit artifacts: ${RUN_DIR}" >&2
  exit "${DOWNLOAD_EXIT_CODE}"
fi

echo "[INFO] DATA-06 official metadata download completed"
echo "[INFO] MD5, SHA256, and exact byte sizes all verified"
