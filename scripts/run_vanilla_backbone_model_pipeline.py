...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--portable-lock-evidence", action="store_true")
...
def validate_paths(
    ...
    *,
    ...
    expected = (
        output_dir.parent / f"{backbone_id}.portable_model_lock.json"
        if portable_lock_evidence
        else REPOSITORY_ROOT / "results/model_locks" / f"{backbone_id}.json"
    ).resolve()
...
def main() -> int:
    ...
    try:
        ...
        if args.portable_lock_evidence:
            lock_command.append("--portable-lock-evidence")
        run_stage(
            name="model_lock",
