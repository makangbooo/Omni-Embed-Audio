...
if [[ "$#" -ne 1 ]]; then
  echo "Usage: $0 <completed-m2d-clap-clotho-main-result-directory>" >&2
  exit 2
fi
...
MODEL_LOCK="${MAIN_DIR}/m2d_clap.portable_model_lock.json"
BASELINE_DIR="${MAIN_DIR}/embeddings"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_ID="m2d_clap_clotho_positive_uiq_${STAMP}"
...
echo "EXPERIMENT_NAME=M2D-CLAP Clotho positive UIQ Tables 12-15"
...
echo "OEA_SOURCE_FILES=AudioRetrieval/models/m2d_clap_adapter.py; AudioRetrieval/models/portable_m2d.py; AudioRetrieval/preprocessing/embeddings/uiq_text.py; scripts/evaluate_official_source_oea_clotho.py"
...
for required in "${CHECKPOINT}" "${BERT_TOKENIZER}" "${CAPTIONS_CSV}" \
...
if [[ "$(<"${MAIN_DIR}/exit_code.txt")" != "0" ]]; then
  fail 4 "The supplied M2D-CLAP main run is not complete"
fi
...
python -m AudioRetrieval preprocess uiq-embeddings \
  ...
  --output-dir "${UIQ_DIR}" \
  || fail $? "M2D-CLAP UIQ embedding generation failed"
...
python scripts/evaluate_official_source_oea_clotho.py \
  ...
  --model "M2D-CLAP" \
  --seed 0 \
  || fail $? "M2D-CLAP positive UIQ metric finalization failed"
echo "STAGE_END=${CURRENT_STAGE} RC=0"
