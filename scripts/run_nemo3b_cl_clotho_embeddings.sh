...
if [[ -z "${SMOKE_METRICS:-}" ]]; then
  echo "[ERROR] SMOKE_METRICS must point to a complete Nemo 5/25 smoke generation_metrics.json." >&2
  exit 2
fi
...
PROTOCOL_CONFIG="${ROOT_DIR}/configs/eval/nemo3b_cl_clotho_embeddings.json"
MODEL_LOCK="${MODEL_LOCK:-${ROOT_DIR}/results/model_locks/oea_nemo3b_cl.json}"
RUN_ID="${RUN_ID:-oea_nemo3b_clotho_embeddings_seed42_$(date +%Y%m%d_%H%M%S)}"
...
{
  printf 'MODEL_ROOT=%q DATA_ROOT=%q MODEL_LOCK=%q SMOKE_METRICS=%q RUN_ID=%q ' \
    "${MODEL_ROOT}" "${DATA_ROOT}" "${MODEL_LOCK}" "${SMOKE_METRICS}" "${RUN_ID}"
  printf '%q ' "$0" "$@"
  printf '\n'
...
echo "[INFO] Nemo full Clotho run ID: ${RUN_ID}"
