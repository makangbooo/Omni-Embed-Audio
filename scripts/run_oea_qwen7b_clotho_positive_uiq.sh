...
case "${VARIANT_ID}" in
  oea_qwen7b)
    ...
    CHECKPOINT_NAME="step_300.pt"
    CHECKPOINT_BYTES=17940602533
    MAIN_RUN_ID="official_source_oea_qwen7b_clotho_main_20260730_002924"
    ;;
  oea_qwen7b_cl)
    PAPER_MODEL="OEA-Qwen7B (+Cl)"
    RELEASED_MODEL="OEA-Qwen7B-Cl"
    CHECKPOINT_NAME="step_330.pt"
    CHECKPOINT_BYTES=17940602661
    MAIN_RUN_ID="official_source_oea_qwen7b_cl_clotho_main_20260730_004037"
    ;;
  *)
    echo "[ERROR] Variant must be oea_qwen7b or oea_qwen7b_cl." >&2
    exit 2
    ;;
esac
...
CHECKPOINT="${MODEL_ROOT}/${RELEASED_MODEL}/${CHECKPOINT_NAME}"
CAPTIONS_CSV="${DATA_ROOT}/clotho_v2.1/source/clotho_captions_evaluation.csv"
MAIN_DIR="${ROOT_DIR}/results/raw/${MAIN_RUN_ID}"
BASELINE_DIR="${MAIN_DIR}/embeddings"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_ID="${VARIANT_ID}_clotho_positive_uiq_${STAMP}"
...
echo "EXPERIMENT_NAME=${PAPER_MODEL} Clotho positive UIQ Tables 12-15"
...
echo "MODEL=${PAPER_MODEL}; released checkpoint ${RELEASED_MODEL}/${CHECKPOINT_NAME}"
...
echo "OEA_SOURCE_FILES=AudioRetrieval/preprocessing/embeddings/oea.py; AudioRetrieval/preprocessing/embeddings/uiq_text.py; scripts/evaluate_official_source_oea_clotho.py"
...
for required in "${BASE_MODEL_DIR}" "${CHECKPOINT}" "${CAPTIONS_CSV}" \
  "${MAIN_DIR}/exit_code.txt" "${BASELINE_DIR}/audio_embeddings.npz" \
...
if [[ "$(<"${MAIN_DIR}/exit_code.txt")" != "0" ]]; then
  fail 4 "The fixed ${PAPER_MODEL} main run is not complete"
fi
if [[ "$(stat -c %s "${CHECKPOINT}")" != "${CHECKPOINT_BYTES}" ]]; then
  fail 4 "Official checkpoint size does not match the pinned manifest"
fi
...
python -m AudioRetrieval preprocess uiq-embeddings \
  ...
  --output-dir "${UIQ_DIR}" \
  || fail $? "${PAPER_MODEL} UIQ embedding generation failed"
...
python scripts/evaluate_official_source_oea_clotho.py \
  ...
  --model "${PAPER_MODEL}" \
  --seed 0 \
  || fail $? "${PAPER_MODEL} positive UIQ metric finalization failed"
echo "STAGE_END=${CURRENT_STAGE} RC=0"

sha256sum "${UIQ_DIR}"/uiq_*_embeddings.npz \
