...
if [[ "$#" -ne 1 ]]; then
  echo "Usage: $0 <completed-mga-clap-clotho-main-result-directory>" >&2
  exit 2
fi
...
MODEL_LOCK="${MAIN_DIR}/mga_clap.portable_model_lock.json"
BASELINE_DIR="${MAIN_DIR}/embeddings"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_ID="mga_clap_clotho_positive_uiq_${STAMP}"
...
echo "EXPERIMENT_NAME=MGA-CLAP Clotho positive UIQ Tables 12-15"
...
for required in "${SOURCE_DIR}" "${CHECKPOINT}" "${BERT_TOKENIZER}" \
  "${CAPTIONS_CSV}" "${MODEL_LOCK}" \
  "${MAIN_DIR}/exit_code.txt" \
...
if [[ "$(<"${MAIN_DIR}/exit_code.txt")" != "0" ]]; then
  fail 4 "The supplied MGA-CLAP main run is not complete"
fi
...
PY
)" || fail 5 "Could not read checkpoint hash from model lock"
...
python -m AudioRetrieval preprocess uiq-embeddings \
  ...
  --output-dir "${UIQ_DIR}" \
  || fail $? "MGA-CLAP UIQ embedding generation failed"
...
python scripts/evaluate_official_source_oea_clotho.py \
  ...
  --model "MGA-CLAP" \
  --seed 0 \
  || fail $? "MGA-CLAP positive UIQ metric finalization failed"
echo "STAGE_END=${CURRENT_STAGE} RC=0"
