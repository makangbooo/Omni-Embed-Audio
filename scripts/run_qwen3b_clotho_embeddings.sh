...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <completed_smoke_generation_metrics.json>" >&2
  exit 2
fi
...
RUN_ID="${RUN_ID:-oea_qwen3b_ac_clotho_embeddings_seed42_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "${SMOKE_METRICS}" ]]; then
  echo "[ERROR] Explicit completed smoke metrics are missing: ${SMOKE_METRICS}" >&2
  exit 4
fi
if [[ ! -f "${MODEL_LOCK}" ]]; then
  echo "[ERROR] Committed OEA model lock is missing: ${MODEL_LOCK}" >&2
  exit 4
fi
if ! git ls-files --error-unmatch -- "${MODEL_LOCK#${ROOT_DIR}/}" >/dev/null 2>&1; then
  echo "[ERROR] OEA model lock must be committed before GPU generation: ${MODEL_LOCK}" >&2
  exit 4
fi
...
echo "[INFO] OEA-Qwen3B full Clotho run ID: ${RUN_ID}"
echo "[INFO] Output directory: ${OUTPUT_DIR}"
echo "[INFO] Attempt directory: ${ATTEMPT_DIR}"
echo "[INFO] Smoke evidence: ${SMOKE_METRICS}"
echo "[INFO] Fixed workload: 1,045 audio candidates and 5,225 caption queries"
echo "[INFO] Resume by exporting the same RUN_ID and running this wrapper again."

python scripts/verify_oea_smoke_gate.py \
  --metrics "${SMOKE_METRICS}" \
  --model-lock "${MODEL_LOCK}" \
  --full-protocol-config "${PROTOCOL_CONFIG}" \
