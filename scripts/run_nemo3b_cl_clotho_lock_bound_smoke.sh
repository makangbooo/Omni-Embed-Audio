...
PROTOCOL_CONFIG="${ROOT_DIR}/configs/eval/nemo3b_cl_clotho_lock_bound_smoke_embeddings.json"
MODEL_LOCK="${MODEL_LOCK:-${ROOT_DIR}/results/model_locks/oea_nemo3b_cl.json}"
RUN_ID="${RUN_ID:-oea_nemo3b_clotho_lock_bound_smoke_seed42_$(date +%Y%m%d_%H%M%S)}"
...
{
  printf 'MODEL_ROOT=%q MODEL_LOCK=%q RUN_ID=%q ' \
    "${MODEL_ROOT}" "${MODEL_LOCK}" "${RUN_ID}"
  printf '%q ' "$0" "$@"
  printf '\n'
...
if [[ -s "${ATTEMPT_DIR}/git_status.txt" ]]; then
  echo "[ERROR] Lock-bound smoke requires a clean Git worktree." >&2
  exit 3
fi
...
echo "[INFO] Nemo lock-bound smoke run ID: ${RUN_ID}"
echo "[INFO] Output directory: ${OUTPUT_DIR}"
echo "[INFO] Attempt directory: ${ATTEMPT_DIR}"
echo "[INFO] Fixed workload: 5 audio candidates and 25 caption queries"
echo "[INFO] Network disabled; pinned local custom model code is lock-verified"
