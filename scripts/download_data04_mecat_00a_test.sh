...
RUN_ID="data04_mecat_00a_test_download_$(date +%Y%m%d_%H%M%S)"
RUN_DIR="${ROOT_DIR}/logs/${RUN_ID}"
MANIFEST="${ROOT_DIR}/configs/resources/data04_mecat_00a_test.json"
...
echo "[INFO] Pinned MECAT 00A test download: 173,168,424-byte shard plus README"
...
du -sh "${DATA_ROOT}/mecat_caption_be4a24c3" \
  > "${RUN_DIR}/dataset_size.txt" 2>&1 || true

if [[ "${DOWNLOAD_EXIT_CODE}" -ne 0 ]]; then
  echo "[ERROR] DATA-04 download failed; partial files were preserved for resume." >&2
  echo "[ERROR] Audit artifacts: ${RUN_DIR}" >&2
  exit "${DOWNLOAD_EXIT_CODE}"
fi

echo "[INFO] DATA-04 download and LFS SHA256 verification completed"
