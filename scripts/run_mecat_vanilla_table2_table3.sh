...
SOURCE_MANIFEST="${DATA_ROOT}/mecat_caption_be4a24c3/manifests/mecat_00a_test_manifest.jsonl"
SOURCE_MANIFEST_SHA256="b4c4d8c1c5928ba5b23a25b3169519871cb2438dfed05823610aebeccb8594a6"
MODEL_LOCK="${ROOT_DIR}/results/model_locks/${BACKBONE_ID}.json"
PROTOCOL_CONFIG="${ROOT_DIR}/configs/eval/${BACKBONE_ID}_mecat_embeddings.json"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_ID="${BACKBONE_ID}_mecat_table2_table3_public848_short_${STAMP}"
RESULT_DIR="${ROOT_DIR}/results/raw/${RUN_ID}"
GENERATION_DIR="${RESULT_DIR}/${BACKBONE_ID}_mecat_embeddings_seed42_${STAMP}"
NPZ_DIR="${RESULT_DIR}/embeddings"
METRICS_DIR="${RESULT_DIR}/metrics"
ATTEMPT_DIR="${RESULT_DIR}/attempt"
PROJECTED_MANIFEST="${RESULT_DIR}/mecat_short_all_manifest.jsonl"
...
echo "EXPERIMENT_NAME=${PAPER_MODEL} vanilla MECAT Tables 2 and 3 public 848-row short-all"
...
echo "TOTAL_WORKLOAD=848 audio + 2,544 caption embeddings + 4 retrieval protocols"
...
for required in "${SOURCE_MANIFEST}" "${MODEL_LOCK}" "${PROTOCOL_CONFIG}" "${ROOT_DIR}/${EVIDENCE}"; do
  [[ -e "${required}" ]] || fail 4 "Required resource is missing: ${required}"
done
[[ "$(sha256sum "${SOURCE_MANIFEST}" | awk '{print $1}')" == "${SOURCE_MANIFEST_SHA256}" ]] \
  || fail 4 "MECAT source manifest SHA256 mismatch"
...
CURRENT_STAGE="manifest_projection"
run_stage "${CURRENT_STAGE}" 120 \
  python scripts/prepare_vanilla_mecat_manifest.py \
    --source-manifest "${SOURCE_MANIFEST}" \
    --source-sha256 "${SOURCE_MANIFEST_SHA256}" \
    --output "${PROJECTED_MANIFEST}" \
  || fail $? "Vanilla MECAT short-all manifest projection failed"
...
run_stage "${CURRENT_STAGE}" 30 \
  ...
  || fail $? "Vanilla MECAT config resolution failed"
...
run_stage "${CURRENT_STAGE}" 5400 \
  python scripts/generate_vanilla_backbone_embeddings.py \
    ...
    --manifest "${PROJECTED_MANIFEST}" --output-dir "${GENERATION_DIR}" \
    --attempt-dir "${ATTEMPT_DIR}" \
  || fail $? "Vanilla MECAT embedding generation failed"
...
run_stage "${CURRENT_STAGE}" 300 \
  python scripts/export_vanilla_audiocaps_npz.py \
    ...
    --expected-candidates 848 --captions-per-audio 3 --dataset-name MECAT \
  || fail $? "Vanilla MECAT canonical export failed"

CURRENT_STAGE="cpu_table2_table3_metrics"
export CUDA_VISIBLE_DEVICES=""
run_stage "${CURRENT_STAGE}" 300 \
  python scripts/evaluate_mecat_table2_table3.py \
    --audio-embedding-dir "${NPZ_DIR}" \
    --caption-embedding-dir "${NPZ_DIR}" \
    --output-dir "${METRICS_DIR}" --model "${PAPER_MODEL}" \
    --expected-candidates 848 --paper-candidates 847 \
    --captions-per-audio 3 --seed 0 \
  || fail $? "Vanilla MECAT Table 2/3 finalization failed"

sha256sum "${PROJECTED_MANIFEST}" "${RESOLVED_CONFIG}" \
  "${GENERATION_DIR}/generation_metrics.json" "${NPZ_DIR}/conversion_metrics.json" \
  "${NPZ_DIR}/audio_embeddings.npz" "${NPZ_DIR}/caption_embeddings.npz" \
...
sha256sum "${LOG_FILE}" >> "${RESULT_DIR}/artifact_sha256.txt"
