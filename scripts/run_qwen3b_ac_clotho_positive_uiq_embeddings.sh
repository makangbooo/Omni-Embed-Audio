...
CONFIG="${UIQ_EMBEDDING_CONFIG:-${ROOT_DIR}/configs/eval/qwen3b_clotho_positive_uiq_embeddings.json}"
BASE_PROTOCOL_CONFIG="${BASE_PROTOCOL_CONFIG:-${ROOT_DIR}/configs/eval/qwen3b_clotho_embeddings.json}"
MODEL_LOCK="${MODEL_LOCK:-${ROOT_DIR}/results/model_locks/oea_qwen3b.json}"
RUN_PREFIX="${UIQ_RUN_PREFIX:-oea_qwen3b_ac_clotho_positive_uiq_embeddings_seed42}"
RUN_ID="${RUN_ID:-${RUN_PREFIX}_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${RESULT_ROOT:-${ROOT_DIR}/results/raw}/${RUN_ID}"
RESOLVED_BASE_CONFIG="${OUTPUT_DIR}/resolved_base_embedding_config.json"
...
if [[ -s "${ATTEMPT_DIR}/git_status.txt" ]]; then
  echo "[ERROR] Formal UIQ embedding generation requires a clean Git worktree." >&2
  exit 3
fi
...
python scripts/build_official_oea_eval_config.py \
  --protocol-config "${BASE_PROTOCOL_CONFIG}" \
  --model-lock "${MODEL_LOCK}" \
  --output "${RESOLVED_BASE_CONFIG}" \
  > "${ATTEMPT_DIR}/config_resolution.json"

python scripts/generate_oea_uiq_embeddings.py \
  --config "${CONFIG}" \
  --base-embedding-config "${RESOLVED_BASE_CONFIG}" \
